...
def build_fact_cea(cea: pd.DataFrame, dim_ciudadano: pd.DataFrame) -> pd.DataFrame:
    """FACT_CEA_CLASES: hecho granular de cada clase."""
    fact = cea.merge(dim_ciudadano, on="ID_ciudadano", how="left")
    cols = ["sk_ciudadano", "ID_ciudadano", "clase_norm", "horas",
            "instructor_norm", "fecha_date", "es_practica",
            "horas_acum_ciudadano", "_source", "_ingested_at"]
    return fact[[c for c in cols if c in fact.columns]].reset_index(drop=True)


def build_fact_crc(crc: pd.DataFrame, dim_ciudadano: pd.DataFrame) -> pd.DataFrame:
    """FACT_CRC_EXAMENES: hecho granular de cada examen."""
    fact = crc.merge(dim_ciudadano, on="ID_ciudadano", how="left")
    cols = ["sk_ciudadano", "ID_ciudadano", "tipo_examen_norm",
            "resultado_aprobado", "fecha_date",
            "examenes_aprobados_acum", "_source", "_ingested_at"]
    return fact[[c for c in cols if c in fact.columns]].reset_index(drop=True)


def build_dim_runt(runt: pd.DataFrame, dim_ciudadano: pd.DataFrame) -> pd.DataFrame:
    """DIM_RUNT: estado más reciente de licencia por ciudadano."""
    dim = runt.merge(dim_ciudadano, on="ID_ciudadano", how="left")
    cols = ["sk_ciudadano", "ID_ciudadano", "estado_licencia_norm",
            "licencia_activa", "fecha_actualizacion_date",
            "dias_desde_actualizacion", "_source", "_ingested_at"]
    return dim[[c for c in cols if c in dim.columns]].reset_index(drop=True)
...
def run_transformations(ingestion_results: dict) -> dict:
    ...
    fact_cea      = build_fact_cea(cea_cur, dim_ciudadano)
    fact_crc      = build_fact_crc(crc_cur, dim_ciudadano)
    dim_runt      = build_dim_runt(runt_cur, dim_ciudadano)
